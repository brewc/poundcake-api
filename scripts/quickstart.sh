...
if docker-compose -f docker-compose.yml exec -T api python src/app/scripts/init_database.py; then
    print_success "PoundCake database initialized"
else
    print_warning "Database initialization may have failed"
    print_info "You can run manually: docker-compose -f docker-compose.yml exec api python src/app/scripts/init_database.py"
fi
